  ...
  return (
    <Section id="services" className="py-20">
      <div className="flex items-end justify-between mb-10">
        <h2 className="text-3xl sm:text-4xl font-semibold">Υπηρεσίες</h2>
        <Button variant="ghost" onClick={()=>scrollToId('contact')} className="gap-2">
          Ζήτα παρουσίαση <ArrowRight className="h-4 w-4" />
        </Button>
      </div>
  ...
  return (
    <Section id="about" className="py-20">
      <div className="grid lg:grid-cols-2 gap-10 items-center">
        <div>
          <h2 className="text-3xl sm:text-4xl font-semibold">Ποιοι είμαστε</h2>
...
function Projects(){
  ...
  return (
    <Section id="projects" className="py-20">
      <div className="flex items-end justify-between mb-10">
        <h2 className="text-3xl sm:text-4xl font-semibold">Ενδεικτικά Έργα</h2>
        <Button variant="ghost" onClick={()=>scrollToId('contact')} className="gap-2">
          Ζήτα portfolio <ArrowRight className="h-4 w-4" />
        </Button>
      </div>
      <div className="grid md:grid-cols-3 gap-6">
        {items.map((p,i)=>(
...
function Sustainability(){
  const kpis = [['A+','Ενεργειακή Κλάση'],['≤ 0.30','U‑value τοιχοποιίας'],['> 4.5','SCOP/SEER στόχοι']]
      ...
      <div className="grid lg:grid-cols-2 gap-10">
        <div>
          ...
          <p className="mt-6 text-neutral-700 max-w-xl">
            Θέλεις να συζητήσουμε ένα οικόπεδο ή αναβάθμιση κατοικίας; Στείλε μας λίγες γραμμές και θα επανέλθουμε με προτάσεις.
          </p>
          <div className="mt-6 space-y-3 text-neutral-800">
            ...
            <div className="flex items-center gap-2"><MapPin className="h-4 w-4"/> Αθήνα, Ελλάδα</div>
          </div>
        </div>
        <form action="/api/contact" method="POST" className="card p-6 grid gap-4">
          <div className="grid sm:grid-cols-2 gap-4">
            <input name="name" className="rounded-xl border px-3 py-2 outline-none focus:ring-2 focus:ring-[var(--brass)]" placeholder="Όνομα" required />
          ...
          <input name="phone" className="rounded-xl border px-3 py-2 outline-none focus:ring-2 focus:ring-[var(--brass)]" placeholder="Τηλέφωνο (προαιρετικό)" />
          <textarea name="message" rows={5} className="rounded-xl border px-3 py-2 outline-none focus:ring-2 focus:ring-[var(--brass)]" placeholder="Πες μας δυο λόγια για το project σου" required />
          <Button className="self-start">Αποστολή</Button>
          <p className="text-xs text-neutral-500">Με την αποστολή αποδέχεσαι τους όρους χρήσης και την πολιτική απορρήτου.</p>
        </form>
      </div>
    </Section>
  )
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold leading-tight">
                ...
                <span className="block mt-2 gradient-title">διαχρονική πολυτέλεια</span>
                {' και καθαρό αποτύπωμα.'}
              </h1>
              <p className="mt-6 text-lg text-neutral-700 max-w-xl">
                Η MODEN αναπτύσσει και εκσυγχρονίζει βιώσιμες βίλες, μεζονέτες και διαμερίσματα. Minimal αισθητική, premium υλικά, και λύσεις net‑zero εμπνευσμένες από την καθημερινή ζωή.
              </p>
              <div className="mt-8 flex flex-wrap gap-3">
                <Button size="lg" className="rounded-2xl" onClick={()=>document.getElementById('projects')?.scrollIntoView({behavior:'smooth'})}>Δες Έργα</Button>
                <Button size="lg" variant="outline" className="rounded-2xl" onClick={()=>document.getElementById('services')?.scrollIntoView({behavior:'smooth'})}>Τι προσφέρουμε</Button>
              </div>
              <div className="mt-10 flex items-center gap-6 text-sm text-neutral-700">
                ...
                <div className="flex items-center gap-2"><Leaf className="h-4 w-4"/> Net‑zero προσέγγιση</div>
                <div className="flex items-center gap-2"><Building2 className="h-4 w-4"/> Turn‑key λύσεις</div>
              </div>
            </motion.div>
